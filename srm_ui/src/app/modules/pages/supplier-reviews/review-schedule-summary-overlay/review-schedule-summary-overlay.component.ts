...
@Component({
  selector: 'review-schedule-summary',
  templateUrl: './review-schedule-summary-overlay.component.html',
  encapsulation: ViewEncapsulation.None
})
export class ReviewScheduleSummaryOverlayComponent {
  ...
  issuccess = false;
  iserror = false;
  dialogTitle = "Supplier Review Scheduling Summary"
  submitButtonText = "Schedule Review"
  ...
  supplierNameList = []
  constructor(@Inject(MAT_DIALOG_DATA) public data, public dialogRef: MatDialogRef<ReviewScheduleSummaryOverlayComponent>,
    public dialog: MatDialog, private http: HttpClient
  ) {
    this.reviewInitiationForm = data.value;
    var reviewInitiationForm = this.reviewInitiationForm
  ...
  ngOnInit(): void {
    if (this.editSession == 1) {
      this.dialogTitle = "Review Schedule Edit Summary"
      this.submitButtonText = "Edit Review Schedule"
    }
