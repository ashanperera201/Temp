...
const product_DATA_CATEGORIES_NEW: Dashboardproduct[] = [];

@Component({
  selector: 'app-itemreportall-datatable',
  templateUrl: './item-reportall-datatable.component.html',
  styleUrls: ['./item-reportall-datatable.component.scss'],
  providers: [DatePipe]
})
export class ItemReportAllDatatableComponent implements OnInit {
  @ViewChild('tableOnePaginator', { read: MatPaginator }) tableOnePaginator: MatPaginator;

  dataSourceDashboard = new MatTableDataSource(product_DATA_CATEGORIES_NEW);
  tempData: Dashboardproduct[];

  currentReqPage = '';
  isloading = true;

  displayedDashboardColumns: string[] = ['position', 'supplierName', 'criticality', 'status', 'currentPosition', 'country', 'createddate',
    'establishmentyear', 'issuedby', 'city', 'postalcode', 'adressline1', 'firstname', 'email', 'crno', 'typeoforg', 'vatno', "pushsupplierstatus", "invitedsupplier"];
  ...
  ngOnInit(): void {
    ...
    this.filterValues = JSON.parse(localStorage.getItem('reportfilter'));
    this.filterValues = this.filterValues == null ? {} : this.filterValues;
  }

  // Get all the suppliers
  getAllSuppliers() {
    this.http.get<any>(environment.nodeurl + '/api/supplier/allsuppliers').subscribe(async data => {
      this.AllData = data;

      this.dataSourceDashboard.data = [];
      this.addSuppliersData(this.AllData);
    });
  }
  ...
  addSuppliersData(suppliersData) {
    ...
    if (count == suppliersData.length) {
      this.dataSourceDashboard = new MatTableDataSource<Dashboardproduct>(this.dataSourceDashboardList);
  ...
  filterChange(filter, event) {
    ...
    localStorage.setItem('reportfilter', JSON.stringify(this.filterValues));
    ...
    for (let key in this.filterValues) {
      ...
      const temp = this.dataSourceDashboard.data.filter(function (d) {
        ...
        else if ((type != "createddatefrom" && type != "createddateto") && (type != "establishmentyearfrom" && type != "establishmentyearto")) {
  ...
  openRowInfo(position) {
    this.router.navigate(['/dashboard/inner/r/' + position]);
  }
  ...
  setFilterText() {
    ...
    this.PushSupplierStatusVal = this.filterValues['pushsupplierstatus'] ? this.filterValues['pushsupplierstatus'] : '';
    this.InvitedSupplierVal = this.filterValues['invitedsupplier'] ? this.filterValues['invitedsupplier'] : '';
        ...
        if (count == 0) {
          for (var i = 0; i < this.dataSourceDashboard.filteredData.length; i++) {
            this.DataToExport.push({
              ...
              // RegDate: moment(new Date(this.dataSourceDashboard.filteredData[i].regDate)).format('YYYY-MM-DD'),
              ...
              // AuditCreatedDate: moment(new Date(this.dataSourceDashboard.filteredData[i].auditcreateddate)).format('YYYY-MM-DD HH:mm'),
              ...
              CreatedDate: moment(new Date(this.dataSourceDashboard.filteredData[i].createddate)).format('YYYY-MM-DD HH:mm'),
